#!/bin/bash
# YouTube Ads MCP Skill - Health Check

echo "🔍 Testing YouTube Ads MCP Skill Setup..."
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.template ]; then
        cp .env.template .env
        echo "📋 Created .env from template. Please add your Google API credentials."
    else
    ...
    required_vars=("GOOGLE_CLIENT_ID" "GOOGLE_CLIENT_SECRET" "GOOGLE_REFRESH_TOKEN" "GOOGLE_ADS_ACCOUNT_ID")
    missing_vars=()
...
echo "\n🎯 YouTube Ads MCP Skill is ready!"
echo "Next steps:"
echo "1. Add your Google API credentials to .env"
echo "2. Run ./install.sh to install dependencies"
echo "3. Test with: python3 scripts/youtube_ads_manager.py --help"
