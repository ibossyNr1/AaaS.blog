#!/bin/bash
# Twitter/X Ads MCP Skill - Health Check

echo "🔍 Testing Twitter/X Ads MCP Skill Setup..."
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.template ]; then
        cp .env.template .env
        echo "📋 Created .env from template. Please add your Twitter/X API credentials."
    else
    ...
    required_vars=("TWITTER_API_KEY" "TWITTER_API_SECRET" "TWITTER_ACCESS_TOKEN" "TWITTER_ACCESS_SECRET" "TWITTER_ADS_ACCOUNT_ID")
    missing_vars=()
...
echo "\n🎯 Twitter/X Ads MCP Skill is ready!"
echo "Next steps:"
echo "1. Add your Twitter/X API credentials to .env"
echo "2. Run ./install.sh to install dependencies"
echo "3. Test with: python3 scripts/twitter_ads_manager.py --help"
