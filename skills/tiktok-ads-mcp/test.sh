#!/bin/bash
# TikTok Ads MCP Skill - Health Check

echo "🔍 Testing TikTok Ads MCP Skill Setup..."
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.template ]; then
        cp .env.template .env
        echo "📋 Created .env from template. Please add your TikTok API credentials."
    else
    ...
    required_vars=("TIKTOK_ACCESS_TOKEN" "TIKTOK_APP_ID" "TIKTOK_SECRET" "TIKTOK_ADVERTISER_ID")
    missing_vars=()
...
echo "\n🎯 TikTok Ads MCP Skill is ready!"
echo "Next steps:"
echo "1. Add your TikTok API credentials to .env"
echo "2. Run ./install.sh to install dependencies"
echo "3. Test with: python3 scripts/tiktok_ads_manager.py --help"
